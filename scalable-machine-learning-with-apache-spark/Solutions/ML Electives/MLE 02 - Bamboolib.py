...
# Step: Create new column 'log_price' from formula 'np.log(price)'
airbnb_pd['log_price'] = np.log(airbnb_pd['price'])
...
import pandas as pd; import numpy as np
# Step: Drop columns
airbnb_reduced = airbnb_pd.drop(columns=['bedrooms', 'beds', 'price'])

# Step: Create new column 'room_type_ord' from formula 'room_type.map({"Shared room" : 0, "Private room" : 1, "Entire home/apt" : 2})'
airbnb_reduced['room_type_ord'] = airbnb_reduced['room_type'].map({"Shared room" : 0, "Private room" : 1, "Entire home/apt" : 2})
